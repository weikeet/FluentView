package com.weiwei.fluent.widget
...
abstract class AbsFluentLayout : ViewGroup {
  ...
  protected fun View.autoMeasure() {
    if (isGone) return
    measure(
      this.defaultWidthMeasureSpec(parentView = this@AbsFluentLayout),
      this.defaultHeightMeasureSpec(parentView = this@AbsFluentLayout)
    )
  }
  ...
  protected fun View.layout(x: Int, y: Int, fromRight: Boolean = false) {
    ...
    if (!fromRight) {
      ...
      layout(this@AbsFluentLayout.measuredWidth - x - measuredWidth, y)
    }
  }
